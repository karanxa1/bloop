...
- [x] landing committed (906b1b0): prebuilt tailwind css, csp-safe · ci actions pinned to shas (3d9c050)
- [~] **attachments** — files/folders/images in chat → r2 `ws/<conv>/uploads/…` (sandbox hydrates automatically), vision for images, `/upload` + `/raw` routes (core-agent, after security) · paperclip/drag-drop/paste + progress chips + history chips (ui-a) · contract in docs/contracts-v3.md
